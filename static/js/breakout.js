...
Breakout = {
    ...
    Defaults: {
        ...
        keys: [
            {
                keys: [Game.KEY.LEFT, Game.KEY.A], mode: 'down', action: function () {
                    this.paddle.moveLeft();
                }
            },
            {
                keys: [Game.KEY.RIGHT, Game.KEY.D], mode: 'down', action: function () {
                    this.paddle.moveRight();
                }
            },
            {
                keys: [Game.KEY.LEFT, Game.KEY.A], action: function () {
                    this.paddle.stopMovingLeft();
                }
            },
            {
                keys: [Game.KEY.RIGHT, Game.KEY.D], action: function () {
                    this.paddle.stopMovingRight();
                }
            },
            {
                keys: [Game.KEY.SPACE, Game.KEY.RETURN], state: 'menu', action: function () {
                    this.play();
                }
            },
            {
                keys: [Game.KEY.SPACE, Game.KEY.RETURN], state: 'game', action: function () {
                    this.ball.launchNow();
                }
            },
            {
                key: Game.KEY.ESC, state: 'game', action: function () {
                    this.abandon();
                }
            },
            {
                key: Game.KEY.UP, state: 'menu', action: function () {
                    this.nextLevel();
                }
            },
            {
                key: Game.KEY.DOWN, state: 'menu', action: function () {
                    this.prevLevel();
                }
            }
        ],
    ...
    initialize: function (runner, cfg) {
        ...
        // this.handtracker = Object.construct(Breakout.HandTracker, this);
    ...
    addEvents: function () {
        ...
        }); // prevent ipad bouncing up and down when finger scrolled
    },
    ...
    toggleAccessibility: function () {
        ...
        if (this.accessibility == true) {
            // Load the model.
            this.ball.speed = .2 * this.ball.speed;
            toggleVideo();
        } else {
            console.log("Ending Handtracking");
            // Load the model.
            this.ball.speed = 5 * this.ball.speed;
            toggleVideo();
        }

    },

    accessibleControl: function (predictions) {
        if (predictions[0]) {
            let midval = predictions[0].bbox[0] + (predictions[0].bbox[2] / 2)
            if (midval < 2 * video.width / 5) {
                this.paddle.stopMovingRight();
                this.paddle.moveLeft();
            } else if (midval > 3 * video.width / 5) {
                this.paddle.stopMovingLeft();
                this.paddle.moveRight();
            } else {
                this.paddle.stopMovingLeft();
    ...
    Score: {
        ...
        load: function () {
            this.highscore = this.game.storage.highscore ? parseInt(this.game.storage.highscore) : 1000;
        },
        save: function () {
    ...
    //         };
    //         this.loadModel.call(this);
    //     },
    //
    //     handController: function (predictions) {
    ...
    //             if (predictions[0]) {
    //                 that.handController(predictions);
    //             }
    //             model.renderPredictions(predictions, videocanvas, context, video);
    //             if (isVideo) {
    //                 requestAnimationFrame(that.runDetection.bind(that));
    //             }
    //         });
    //     },
    //
